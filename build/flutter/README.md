...
* `fb` - project identifier (lowercase, no spaces, i.e. "snake_case") used for bundle IDs and package names.
* `` - project description.
* `F&B` - executable or bundle name on disk.
* `Studygram` - display name shown in window titles and about app dialogs.
* `com.bk.studygram` - org name in reverse domain name notation, e.g. `com.mycompany.myproject`.
